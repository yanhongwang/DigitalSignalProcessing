...
color4 = [0 255 0]/255;     % Green 
color5 = [255 0 255]/255;   % Purple
...
if nargin==0 | strcmp(state,'reset')
    ...
    bandw4 = eq_state.BandWidth4;
    centerfreq4 = eq_state.CenterFreq4;
    pk4 = eq_state.peakgain4;
    
    bandw5 = eq_state.BandWidth5;
    centerfreq5 = eq_state.CenterFreq5;
    pk5 = eq_state.peakgain5;
    ...
    [h3,w3] = freqz(b3,a3);
    %Filter4 
    [b4 a4] = peq(0,pk4,centerfreq4,bandw4,NdB);
    [h4,w4] = freqz(b4,a4);
    %Filter5
    [b5 a5] = peq(0,pk5,centerfreq5,bandw5,NdB);
    [h5,w5] = freqz(b5,a5);
    ...
    %--------------------------BAND FOUR------------------------------------------
    %Create the plot
    %line4 = plot(w4*fs/(2*pi),h4,'m','tag','line4');
    line4 = semilogx(w4,h4,'tag','line4');
    set(line4,'color',color4);
    %Set parameters in the Simulink Model
    set_param([Owner,'/PeakGain4'],'value',num2str(pk4));
    set_param([Owner,'/CenterFreq4'],'value',num2str(centerfreq4) );
    set_param([Owner,'/BandWidth4'],'value',num2str(bandw4));
    
    %Add Markers that will be used for manipulating the parameters
    if pk4>0
       [h4max,indexh4] = max(h4);
    else
       [h4max,indexh4] = min(h4);
    end
    w4max = w4(indexh4);
    % the marker is plotted as a line object
    gain4 = line(w4max,h4max,'Marker','o','MarkerEdgeColor','k', ...
        'MarkerFaceColor',color4,'tag','peakgain4','LineStyle','none','markersize',6);
    %-------------------END CODE FOR BAND FOUR--------------------------------------
    
    %--------------------------BAND FIVE------------------------------------------
    %Create the plot
    %line5 = plot(w5*fs/(2*pi),h5,'m','tag','line5');
    line5 = semilogx(w5,h5,'tag','line5');
    set(line5,'color',color5);
    %Set parameters in the Simulink Model
    set_param([Owner,'/PeakGain5'],'value',num2str(pk5));
    set_param([Owner,'/CenterFreq5'],'value',num2str(centerfreq5) );
    set_param([Owner,'/BandWidth5'],'value',num2str(bandw5));
    
    %Add Markers that will be used for manipulating the parameters
    if pk5>0
       [h5max,indexh5] = max(h5);
    else
       [h5max,indexh5] = min(h5);
    end
    w5max = w5(indexh5);
    % the marker is plotted as a line object
    gain5 = line(w5max,h5max,'Marker','o','MarkerEdgeColor','k', ...
        'MarkerFaceColor',color5,'tag','peakgain5','LineStyle','none','markersize',6);
    %-------------------END CODE FOR BAND FIVE--------------------------------------
    ...
    line6 = plot(w3,normSumH,'y--','tag','line6','linewidth', 2,'hittest','off');
    hold off;
    ...
    eqData.H = [h1 h2 h3 h4 h5];
    eqData.centerFreq = [centerfreq1 centerfreq2 centerfreq3 centerfreq4 centerfreq5];
    eqData.bandW = [bandw1 bandw2 bandw3 bandw4 bandw5];
...
elseif strcmp(state,'down')
    ...
    if strcmp(htype,'line')
        ...
        line6 = findobj(gcf,'tag','line6');
        if strcmp(tag,'peakgain1')|strcmp(tag,'peakgain2')|strcmp(tag,'peakgain3')|strcmp(tag,'peakgain4')|strcmp(tag,'peakgain5')
            set(text1,'string','Drag to move the peak around');
        elseif strcmp(tag,'line1')|strcmp(tag,'line2')|strcmp(tag,'line3')|strcmp(tag,'line4')|strcmp(tag,'line5')
            set(text1,'string','Drag to change the bandwidth');
        %elseif strcmp(tag,'line4')
        elseif strcmp(tag,'line6')
...
elseif strcmp(state,'move')
    ...
    line4 = findobj(gcf,'tag','line4');
    gain4 = findobj(gcf,'tag','peakgain4');
    
    line5 = findobj(gcf,'tag','line5');
    gain5 = findobj(gcf,'tag','peakgain5');
    ...
    if strcmp(tag,'peakgain1')|strcmp(tag,'peakgain2')|strcmp(tag,'peakgain3')|strcmp(tag,'peakgain4')|strcmp(tag,'peakgain5')
        ...
        set(line6,'ydata',allH,'xdata',neww);
...
elseif strcmp(state,'save_state')
    ...
    eq_state.peakgain4   =  str2num(get_param([Owner,'/PeakGain4'],'value'));
    eq_state.CenterFreq4 =  str2num(get_param([Owner,'/CenterFreq4'],'value'));
    eq_state.BandWidth4  =  str2num(get_param([Owner,'/BandWidth4'],'value'));
    
    eq_state.peakgain5   =  str2num(get_param([Owner,'/PeakGain5'],'value'));
    eq_state.CenterFreq5 =  str2num(get_param([Owner,'/CenterFreq5'],'value'));
    eq_state.BandWidth5  =  str2num(get_param([Owner,'/BandWidth5'],'value'));
